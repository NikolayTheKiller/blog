...
<!-- ДОБАВИТЬ НОВЫЙ КОММЕНТАРИЙ (НЕ ОТВЕТ НА КОММЕНТ)-->
 <div id="make_comment">
    ...
    <form action="" method="post">
        ...
        <input type="button" value="сохранить" name="submit" onclick="newcomm(ident,username)">
    </form>
 </div>
...
<div id="hideform">
  <form action="" method="post" class="form" style="display: none">
        <h3 style="color: red">Ответить на комментарий</h3>
            ...
            <input type="button" name="submit" value="сохранить"  onclick="replycomment(username)">
  </form>
</div>
<!-- JS -->
<script>
 $(document).ready(function(){});
    ...
    var username='';
    $.post('/tryuser',{},function(data){
       username = JSON.parse(data);
       showcomments(ident,username);
         });
         
    //МЕТОД ДЛЯ ВЫВОДА ВСЕХ КОММЕНТАРИЕВ
    function showcomments(ident,username){
        $.post('/showcomm/'+ident,{},function(data){
    var res = JSON.parse(data);
    for(var i=0; i < res.length; i++){    
    if(res[i]['parent_id']==0){
        if(res[i]['author_id'] !== username){$('#comments').append('<li id="'+res[i]['comment_id']+'" >'+res[i]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[i]['comment_id']+","+res[i]['post_id']+')"></li>');}else{
...
<input type="button" value="Удалить" onclick="deletec('+res[i]['comment_id']+')"></li>');}}
    if(res[i]['parent_id']>0 ){
             if(res[i]['author_id'] !== username){$('#'+res[i]['parent_id']+'').append('<li id="'+res[i]['comment_id']+'" >'+res[i]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[i]['comment_id']+","+res[i]['post_id']+')"></li>');}else{
...
<input type="button" value="Удалить" onclick="deletec('+res[i]['comment_id']+')"></li>');}
 ...
 function newcomm(ident){
    ...
    if(res[0]['role']=='ban'){location="/user/logout";}else{
        if(res[0]['author_id'] !== username){$('#comments').append('<li id="'+res[0]['comment_id']+'" >'+res[0]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[0]['comment_id']+","+res[0]['post_id']+')"></li>')}else{
  ...
  function reply(c_id,post_id){
    ...
    function replycomment(username){
        ...
        $.post('/preply/'+ident,{text:$('#text').val(),parent:comm},function(data){
                var res = JSON.parse(data);
                if(res[0]['role']=='ban'){location="/user/logout";}else{
                if(res[0]['author_id'] !== username){$('#'+res[0]['parent_id']+'').append('<li id="'+res[0]['comment_id']+'" >--->'+res[0]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[0]['comment_id']+","+res[0]['post_id']+')"></li>')}else{    

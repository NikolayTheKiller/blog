...
<script>
 $(document).ready(function(){});
    ...
    $('#comments').append('<li id="'+res[i]['comment_id']+'" >'+res[i]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[i]['comment_id']+","+res[i]['post_id']+')"></li>');}
    if(res[i]['parent_id']>0 ){     
    $('#'+res[i]['parent_id']+'').append('<li id="'+res[i]['comment_id']+'">--------'+res[i]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[i]['comment_id']+","+res[i]['post_id']+')"></li>');
    }}    
 });};
 ...
 function newcomm(ident){
    ...
    var res = JSON.parse(data);
    $('#comments').append('<li id="'+res[0]['comment_id']+'" >'+res[0]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[0]['comment_id']+","+res[0]['post_id']+')"></li>');
    $("#newcom").val('');}
         );};
  ...
  function reply(c_id,post_id){
    ...
    function replycomment(){
        ...
        $.post('/preply/'+ident,{text:$('#text').val(),parent:comm},function(data){
                var res = JSON.parse(data);
                $('#'+res[0]['parent_id']+'').append('<li id="'+res[0]['comment_id']+'" >--->'+res[0]['text']+'<br><input type="button" value="Ответить" class="send" onclick="reply('+res[0]['comment_id']+","+res[0]['post_id']+')"></li>');
                $('.reset').val(''); pop.close();
    });        
    }
